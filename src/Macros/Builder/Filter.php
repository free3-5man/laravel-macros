            ...
            foreach ($whereFields as $table => $fields) {
                ...
                foreach ($fields as $tableField => $requestField) {
                    ...
                    if (!isset($data[$requestField]))
                        $condition = false;
                    elseif (is_array($data[$requestField])) {
                        ...
                        $condition = !empty(array_filter($data[$requestField], function ($item) {
                            return !is_null($item);
                        }));
                    }

                    /** @var Builder $this */
                    $this->when($condition, function ($query) use ($where, $data, $tableField, $requestField, $table) {
                        return $query->{$where}("{$table}.{$tableField}", $data[$requestField]);
                    });
                };
            };
